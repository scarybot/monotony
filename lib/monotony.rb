#!/usr/bin/ruby

require 'pp'
require 'logger'
require 'colorize'
require 'deep_dive'
require 'ruby2d'

require 'monotony/basicproperty'
require 'monotony/chance'
require 'monotony/communitychest'
require 'monotony/game'
require 'monotony/entity'
require 'monotony/player'
require 'monotony/simulatedplayer'
require 'monotony/purchasable'
require 'monotony/square'
require 'monotony/station'
require 'monotony/utility'
require 'monotony/variants'
require 'monotony/decision'
require 'monotony/personality'
require 'monotony/behaviour'
require 'monotony/account'
require 'monotony/transaction'

# The Monopoly Engine!
# @author James Denness <james@recordlive.uk>
module Monotony

end
